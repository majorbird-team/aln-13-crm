...
class SaleCRMPortal(CustomerPortal):
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        values['opportunity_count'] = SaleOrder.search_count(self._prepare_opportunity_domain(partner)) \
            if CrmLead.check_access_rights('read', raise_exception=False) else 0
        
        return values
    
    def _prepare_opportunity_domain(self, partner):
        return [
            ('message_partner_ids', 'child_of', [partner.commercial_partner_id.id])
        ]

    def _crm_check_access(self, opportunity_id, access_token=None):
        opportunity = request.env['crm.lead'].browse([opportunity_id])
        opportunity_sudo = opportunity.sudo()
        try:
            opportunity.check_access_rights('read')
            opportunity.check_access_rule('read')
        except exceptions.AccessError:
            if not access_token or not consteq(picking_sudo.sale_id.access_token, access_token):
                raise
        return opportunity_sudo
